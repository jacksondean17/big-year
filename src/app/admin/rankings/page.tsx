...
import { BtChart, CompCountHistogram, DecisionTimeChart } from "./charts";
...
export default async function AdminRankingsPage() {
  ...
  const rankedChallenges = [...btResult.scores.entries()]
    .map(([id, btScore]) => {
      ...
      return {
        ...
        lnThetaOverride: ch?.ln_theta_override ?? null,
        mappedPoints: ch?.mapped_points ?? null,
      };
    })
  ...
  const labChallenges: LabChallenge[] = rankedChallenges.map((c) => ({
    ...
    lnThetaOverride: c.lnThetaOverride,
    mappedPoints: c.mappedPoints,
  }));
